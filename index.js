...
async function startHisoka() {
    const hisoka = hisokaConnect({
        ...
        browser: ['Mheg Multi Device“, „Safari','1.0.0'],
        auth: state
    })
    ...
    let pa7rick = await hisoka.sendTextWithMentions(tihh.from, `*${hisoka.user.name}* kann keine anrufe empfangen ${tihh.isVideo ? `video` : `suara`}. Es tut uns leid @${tihh.from.split('@')[0]} du wirst geblockt. Wenn Sie es versehentlich öffnen, wenden Sie sich bitte an den Eigentümer !`)
    ...
    // Get Profile Picture Group
       ...
       hisoka.send5ButImg(ciko.id, `「 Gruppeneinstellungen ändern 」\n\nGruppe wurde vom Administrator geschlossen, Jetzt können nur Administratoren Nachrichten senden !`, `Nachricht zur Änderung der Gruppeneinstellungen`, wm_fatih, [])
       ...
       hisoka.send5ButImg(ciko.id, `「 Gruppeneinstellungen ändern 」\n\nGruppeninformationen wurden eingeschränkt, jetzt kann nur der Administrator Gruppeninformationen bearbeiten!`, `Nachricht zur Änderung der Gruppeneinstellungen`, wm_fatih, [])
       } else if (ciko.restrict == false) {
       hisoka.send5ButImg(ciko.id, `「 Gruppeneinstellungen ändern 」\n\nGruppeninfo wurde geöffnet, jetzt können Teilnehmer Gruppeninfo bearbeiten!`, `Nachricht zur Änderung der Gruppeneinstellungen`, wm_fatih, [])
       } else {
       hisoka.send5ButImg(ciko.id, `「 Gruppeneinstellungen ändern 」\n\nGruppenbetreff wurde geändert in *${ciko.subject}*`, `Nachricht zur Änderung der Gruppeneinstellungen`, wm_fatih, [])
     }
    }
    } catch (err){
    console.log(err)
    ...
    hisoka.ev.on('group-participants.update', async (anu) => {
        ...
        try {
            ...
            for (let num of participants) {
                ...
                if (anu.action == 'add') {
                    hisoka.sendMessage(anu.id, { image: { url: ppuser }, mentions: [num], caption: `Willkommen zu ${metadata.subject} @${num.split("@")[0]}` })
                } else if (anu.action == 'remove') {
                    hisoka.sendMessage(anu.id, { image: { url: ppuser }, mentions: [num], caption: `@${num.split("@")[0]} Verlassen von ${metadata.subject}` })
                } else if (anu.action == 'promote') {
                    hisoka.sendMessage(anu.id, { image: { url: ppuser }, mentions: [num], caption: `@${num.split('@')[0]} Fördern von ${metadata.subject}` })
                } else if (anu.action == 'demote') {
                    hisoka.sendMessage(anu.id, { image: { url: ppuser }, mentions: [num], caption: `@${num.split('@')[0]} Herabstufung von ${metadata.subject}` })
              }
            }
        } catch (err) {
            console.log(err)
	...
	for (let i of kon) {
	    list.push({
	    	...
	    	vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${await hisoka.getName(i + '@s.whatsapp.net')}\nFN:${await hisoka.getName(i + '@s.whatsapp.net')}\nitem1.TEL;waid=${i}:${i}\nitem1.X-ABLabel:Ponsel\nitem2.EMAIL;type=INTERNET:hunoter@gmail.com\nitem2.X-ABLabel:Email\nitem3.URL:https://instagram.com/mheg\nitem3.X-ABLabel:Instagram\nitem4.ADR:;; Deutschland;;;;\nitem4.X-ABLabel:Region\nEND:VCARD`
	    })
	}
	hisoka.sendMessage(jid, { contacts: { displayName: `${list.length} Kontakt`, contacts: list }, ...opts }, { quoted })
    }
    ...
    hisoka.ev.on('connection.update', async (update) => {
        ...
        let reason = new Boom(lastDisconnect?.error)?.output.statusCode
            if (reason === DisconnectReason.badSession) { console.log(`Fehlerhafte Sitzungsdatei, bitte Sitzung löschen und erneut scannen`); hisoka.logout(); }
            else if (reason === DisconnectReason.connectionClosed) { console.log("Verbindung geschlossen, neu verbinden...."); startHisoka(); }
            else if (reason === DisconnectReason.connectionLost) { console.log("Verbindung zum Server unterbrochen, Verbindung neu..."); startHisoka(); }
            else if (reason === DisconnectReason.connectionReplaced) { console.log("Verbindung ersetzt, weitere neue Sitzung geöffnet, bitte zuerst aktuelle Sitzung schließen"); hisoka.logout(); }
            else if (reason === DisconnectReason.loggedOut) { console.log(`Gerät abgemeldet, bitte erneut scannen und ausführen.`); hisoka.logout(); }
            else if (reason === DisconnectReason.restartRequired) { console.log("Neustart erforderlich, Neustart..."); startHisoka(); }
            else if (reason === DisconnectReason.timedOut) { console.log("Zeitüberschreitung bei Verbindung, Verbindung neu..."); startHisoka(); }
            else hisoka.end(`Unbekannter Trennungsgrund: ${reason}|${connection}`)
        }
